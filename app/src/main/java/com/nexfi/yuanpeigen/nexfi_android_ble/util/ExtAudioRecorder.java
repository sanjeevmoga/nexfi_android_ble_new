...
/**
 * Created by gengbaolong on 2016/6/7.
 */
public class ExtAudioRecorder {
    ...
    public static ExtAudioRecorder getInstanse(Boolean recordingCompressed) {
        if (recordingCompressed) {
            result = new ExtAudioRecorder(false, MediaRecorder.AudioSource.MIC,
                    sampleRates[3], AudioFormat.CHANNEL_CONFIGURATION_MONO,
                    AudioFormat.ENCODING_PCM_16BIT);
        } else {
            int i = 3;
            do {
                result = new ExtAudioRecorder(true, MediaRecorder.AudioSource.MIC,
...
// * @param snc
// */
//public void recordMedia(StdNetClient snc) {
//// 获取类的实例
//        ExtAudioRecorder recorder = ExtAudioRecorder.getInstanse(false); // 未压缩的录音（WAV）
//        recorder.recordChat(CommonTools.getSDPath() + File.separator,
//        "upload_media.wav");
////录音时间
//        try {
